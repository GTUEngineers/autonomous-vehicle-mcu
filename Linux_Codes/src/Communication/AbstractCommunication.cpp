/**
 *  \file   AbstractCommunication.cpp
...
ErrorStatus AbstractCommunication::receive(std::string &message, int length) {
  if (getConnection() == FALSE) return ErrorStatus::NoSuchConnection;
...
ErrorStatus AbstractCommunication::send(const std::string &message,
                                        int length) {
  if (getConnection() == FALSE) return ErrorStatus::NoSuchConnection;
