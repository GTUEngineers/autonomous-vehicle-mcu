...
bool parse_startstop_req(std::string& req,
...
/*--------------------------< Parse Functions >--------------------------*/
bool parse_startstop(std::string& rep_or_req,
...
bool parse_steering(std::string& rep_or_req, uart::steering_enum& left_or_right,
                    double& angle) {
  uart::pub_sub pubsub;
  if (pubsub.ParseFromArray(rep_or_req.data(), rep_or_req.size())) {
...
bool parse_stateworking(std::string& rep_or_req,
                        uart::stateWorking_enum& active_or_inactive) {
  uart::pub_sub pubsub;
  if (pubsub.ParseFromArray(rep_or_req.data(), rep_or_req.size())) {
...
std::string create_steering_msg(uart::steering_enum left_or_right,
                                double angle) {
  std::string ret_str;
  uart::pub_sub pubsub;
...
std::string create_lidar_rep(float x, float y, float z, float width,
                             float height) {
  std::string ret_str;
  LidarComm::req_rep reqrep;
  std::unique_ptr<LidarComm::Cluster> repdata(new LidarComm::Cluster);
  std::unique_ptr<LidarComm::Position> cluster_position(
      new LidarComm::Position);
...
bool parse_lidar_req(std::string& req, float& x1, float& y1, float& x2,
                     float& y2) {
  LidarComm::req_rep reqrep;
  if (reqrep.ParseFromArray(req.data(), req.size())) {
...
bool parse_lidar_rep(std::string& rep, float& x, float& y, float& z,
...
bool parse_clusters(std::string& subdata,
                    std::vector<std::vector<float>>& clusterdata) {
  LidarComm::pub_sub pubsub;
